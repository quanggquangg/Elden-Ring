...
const ITEMS = [
  ...
  { id: 'stone2', x: 2560, y: 1300, kind: 'stone' },
...
const SPAWNS = [
  ...
  ['soldier', 900, 3000], ['wolf', 2400, 3200], ['wolf', 2450, 3150],
];
const REGIONS = [
...
(function genObstacles() {
  ...
  while (OBST.filter(o => o.kind === 'tree').length < 230 && tries++ < 8000) {
    ...
    OBST.push({ kind: 'tree', x, y, r: tr, cr: tr * 2.6 + r() * 12, golden: r() < 0.22, spr: (r() * 4) | 0 });
  }
  tries = 0;
...
function makeCanopy(cr, golden, seed) {
  ...
  const pal = golden ? ['#9a7526', '#b8912f', '#d4ab45', '#e8c761'] : ['#26331a', '#33431f', '#415227', '#50622d'];
  ...
  hg.addColorStop(0, golden ? 'rgba(255,238,170,.5)' : 'rgba(180,200,120,.25)'); hg.addColorStop(1, 'rgba(0,0,0,0)');
  g.fillStyle = hg; g.beginPath(); g.arc(cx, cx, cr, 0, TAU); g.fill();
  return c;
}
const CANOPY = { green: [], gold: [] };
for (let i = 0; i < 4; i++) { CANOPY.green.push(makeCanopy(52, false, 100 + i)); CANOPY.gold.push(makeCanopy(52, true, 200 + i)); }
...
function defaultSave() {
  return { level: 1, stats: { vig: 10, end: 10, str: 10, mnd: 10 }, runes: 0, weaponLv: 0, flaskMax: 4, lastGrace: 0, discovered: [0], bossDead: false, taken: [], lost: null, treeReached: false, deaths: 0, time: 0 };
}
let S = defaultSave();
...
const G = {
  ...
  touch: false, timers: [], hintT: 0, toast: null,
};
const P = {
...
function applyStats(full) {
  ...
  if (full) { P.hp = P.maxHp; P.st = P.maxSt; P.fp = P.maxFp; P.flasks = S.flaskMax; P.ghost = P.hp; }
  else { P.hp = Math.min(P.hp, P.maxHp); P.st = Math.min(P.st, P.maxSt); P.fp = Math.min(P.fp, P.maxFp); }
}

// ───────────────────────── kẻ địch ─────────────────────────
const LOOK_PLAYER = { body: '#474b52', trim: '#8d9199', head: '#5b5f67', cloak: '#5e1f1c', weapon: 'sword', wlen: 38, wcol: '#dcdcd2', scale: 1 };
const ETYPES = {
  soldier: {
...
function makeBoss() {
  return { isBoss: true, name: 'Varek, Kẻ Canh Cổng Phản Trắc', x: 1400, y: 640, r: 28, hp: 1000, maxHp: 1000, ghost: 1000, face: Math.PI / 2, state: 'dormant', t: 0, cd: 1,
    vx: 0, vy: 0, poise: 170, poiseAcc: 0, lastHit: 9, hurtFlash: 0, phase: 1, atk: null, dead: false, z: 0, elite: true, invuln: 0, anim: 0, stagDur: 0.8, lastMove: '',
...
const stick = { x: 0, y: 0 };
const KEYMAP = { Space: 'roll', KeyJ: 'light', KeyK: 'heavy', KeyL: 'spell', KeyC: 'spell', KeyR: 'flask', KeyE: 'interact', KeyF: 'mount', KeyQ: 'lock' };
function act(a) {
  audioInit();
...
function makeAtk(kind, combo) {
  ...
  if (kind === 'light') {
    ...
    a = { wind: 0.46, act: 0.14, rec: 0.42, mul: 2.2, range: 80, arc: 2.4, lunge: 250, poise: 48, swing: 1, cost: 22 };
  } else {
    a = { wind: 0.08, act: 0.14, rec: 0.3, mul: 1.15, range: 80, arc: 2.8, lunge: 0, poise: 16, swing: 1, cost: 9 };
  }
  return Object.assign(a, { kind, combo, dmg: base * a.mul, hits: new Set(), lunged: false });
}
...
function updatePlayer(dt) {
  ...
  if (p.state === 'idle') {
    ...
    const spd = p.mounted ? 320 : sprint ? 215 : 145;
    if (moving) {
    ...
    if (t < A.wind) {
      ...
      for (const e of targets()) {
        ...
        if (inArc(p.x, p.y, p.face, A.range, A.arc, e.x, e.y, e.r)) { A.hits.add(e); hitEnemy(e, A.dmg, A.poise, p.x, p.y, A.kind); }
      }
    } else {
      if (t > A.wind + A.act + A.rec * 0.35) {
        const a = peekBuf();
        if (a === 'light' && A.kind === 'light' && A.combo < 2) { takeBuf(); startAttack('light', A.combo + 1, moving, mx, my); return; }
...
function hurtPlayer(dmg, fx, fy, heavy) {
  const p = P;
  ...
  p.state = 'hurt'; p.t = 0; p.hurtDur = heavy ? 0.55 : 0.3; p.invuln = 0.4; p.atk = null;
...
function hitEnemy(e, dmg, poise, fx, fy, kind) {
  if (e.dead || (e.invuln || 0) > 0) return;
  let crit = false;
  if (e.state === 'broken') { dmg *= 2.6; crit = true; }
  ...
  burst(e.x, e.y, crit ? 26 : 10, e.isBoss ? '#e8c25e' : '#7c1210', crit ? 220 : 150, 3, 'dot', 0.5, a);
  burst(e.x, e.y, 5, '#fff3c4', 260, 2, 'spark', 0.2, a);
  floatText(e.x, e.y - e.r - 12, String(dmg), crit ? '#ffd36b' : '#f1e6c8', crit);
  if (crit) { SFX.crit(); floatText(e.x, e.y - e.r - 40, 'CHÍ MẠNG', '#ffd36b', true); } else SFX.hit();
  if (!e.isBoss && (e.state === 'idle' || e.state === 'return')) { e.state = 'chase'; e.t = 0; }
  if (!e.isBoss) { const kb = e.elite ? 40 : kind === 'heavy' ? 240 : 120; e.vx += Math.cos(a) * kb; e.vy += Math.sin(a) * kb; }
...
function updateEnemyAtk(e, dt, ang) {
  ...
  if (t < A.wind + A.act) {
    ...
    if (!e.atkHit && P.state !== 'dead' && inArc(e.x, e.y, e.face, A.range, A.arc, P.x, P.y, P.r)) {
      if (hurtPlayer(A.dmg, e.x, e.y, A.dmg >= 45)) e.atkHit = true;
    }
    return;
  }
...
function aoeBlast(x, y, r, dmg) {
  if (dist(x, y, P.x, P.y) < r + P.r) hurtPlayer(dmg, x, y, true);
  aoes.push({ kind: 'flash', x, y, r, t: 0, dur: 0.35 });
}
  ...
  switch (s.k) {
    case 'swing':
      if (t < s.wind) { b.face = turn(b.face, ang, 2.6 * dt); if (cross(s.wind * 0.3)) { addPart(b.x + Math.cos(b.face) * 40, b.y + Math.sin(b.face) * 40 - 20, 0, 0, 0.4, 16, '#fff6d8', 'glint'); SFX.glint(); } }
        ...
        if (!A.hit && inArc(b.x, b.y, b.face, s.range, s.arc, P.x, P.y, P.r) && hurtPlayer(s.dmg, b.x, b.y, true)) A.hit = true;
...
function updateProjs(dt) {
  for (let i = projs.length - 1; i >= 0; i--) {
    ...
    if (Math.random() < 0.8) addPart(q.x, q.y, rand(-10, 10), rand(-10, 10), 0.3, q.kind === 'dagger' ? 2 : 3, q.kind === 'glint' ? '#bcd6ff' : q.kind === 'orb' ? '#8fb0ff' : '#f3cf6e');
    ...
    if (!dead && q.friendly) {
      ...
      if (hurtPlayer(q.dmg, q.x - q.vx * 0.05, q.y - q.vy * 0.05, false)) dead = true;
    }
...
function updateAoes(dt) {
  for (let i = aoes.length - 1; i >= 0; i--) {
    ...
    if (a.kind === 'ring') {
      ...
      if (!a.hit && Math.abs(dist(a.x, a.y, P.x, P.y) - cur) < 16 + P.r && hurtPlayer(a.dmg, a.x, a.y, false)) a.hit = true;
...
function takeItem(it) {
  ...
  else { S.weaponLv++; banner('item', 'Đá Rèn Kiếm', 'Thanh kiếm được cường hóa lên +' + S.weaponLv); }
  save();
}
...
function update(dt) {
  ...
  updatePlayer(dt); updateEnemies(dt); updateBoss(dt); updateProjs(dt); updateAoes(dt); updateParts(dt);
...
function drawWeapon(L, s, wAng, o) {
  ctx.save(); ctx.translate(3 * s, 8 * s); ctx.rotate(wAng);
  const len = L.wlen * s;
...
function drawPlayer() {
  ...
  if (p.state === 'dead') {
    ...
    drawHumanoid(p.x, p.y, p.face, LOOK_PLAYER, 1.2, { alpha: 1 - k * 0.7, kneel: true });
    return;
  }
  if (p.mounted) drawHorse(p.x, p.y, p.face, p.walk);
  let wAng = 0.6, trail = null;
  if (p.state === 'attack' && p.atk) {
    const A = p.atk, t = p.t, sw_ = A.swing;
    if (t < A.wind) wAng = weaponAngle('wind', t / A.wind, sw_);
  ...
  let face = p.face;
  const o = { anim: p.walk, trail, flash: p.invuln > 0.25 };
  ...
  if (p.state === 'roll') {
    ...
    drawHumanoid(p.x, p.y, p.rollDir, LOOK_PLAYER, wAng, o);
    ctx.restore();
  } else {
    drawHumanoid(p.x, p.y - (p.mounted ? 6 : 0), face, LOOK_PLAYER, wAng, o);
  }
...
function drawDecals() {
  ...
  for (const it of ITEMS) {
    ...
    ctx.fillStyle = it.kind === 'seed' ? '#ffe28a' : '#e8f0ff'; ctx.shadowColor = ctx.fillStyle; ctx.shadowBlur = 12;
...
function drawAoeFx() {
  for (const a of aoes) {
    if (a.kind === 'flash') {
      const k = a.t / a.dur;
      ctx.fillStyle = `rgba(255,226,150,${0.5 * (1 - k)})`; ctx.beginPath(); ctx.arc(a.x, a.y, a.r * (0.7 + k * 0.4), 0, TAU); ctx.fill();
    } else if (a.kind === 'ring') {
      const cur = lerp(a.r0, a.r1, a.t / a.dur);
...
function drawProjs() {
  for (const q of projs) {
    if (q.kind === 'dagger') {
      ...
      const col = q.kind === 'glint' ? '#d8e8ff' : '#8fb0ff';
...
function drawCanopies() {
  for (const o of OBST) {
    ...
    const spr = (o.golden ? CANOPY.gold : CANOPY.green)[o.spr];
    let a = 0.96;
...
function render() {
  ...
  for (const e of list) { if (e === P) drawPlayer(); else if (e.isBoss) drawBoss(); else drawEnemy(e); }
  ...
  drawProjs(); drawAoeFx(); drawParts(); drawCanopies(); drawGraceBeams();
...
function drawHUD() {
  ...
  if (P.mounted) { ctx.font = `500 12px ${FONT_U}`; ctx.fillStyle = '#b9d8ff'; ctx.fillText('Đang cưỡi ngựa', fx + fs + 22, fy + 25); }
  // rune
  ...
  // thanh máu boss
  let subY = CH - 110;
  if (G.bossFight && boss && boss.state !== 'dormant') {
    const bw = Math.min(CW - 48, 640), bx = (CW - bw) / 2, by = G.touch ? CH - 250 : CH - 60;
    ctx.font = `600 ${CW < 500 ? 17 : 21}px ${FONT_D}`; ctx.fillStyle = 'rgba(0,0,0,.7)'; ctx.fillText(boss.name, bx + 1, by - 11);
    ctx.fillStyle = '#ece3cc'; ctx.fillText(boss.name, bx, by - 12);
    bar(bx, by, bw, 9, boss.hp / boss.maxHp, boss.ghost / boss.maxHp, '#8e1c16');
    subY = by - 50;
...
function setMode(m) {
  ...
  keys.clear(); stick.x = 0; stick.y = 0;
...
function selectTab(which) {
  ...
  $('paneLevel').hidden = which !== 'level'; $('paneTravel').hidden = which !== 'travel';
}
